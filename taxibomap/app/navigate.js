...
async function initializeNavigation() {
  ...
  map.on("load", async () => {
    ...
    if (routes.length) {
      ...
      nextInstruction.textContent = "Record or import a route first";
    }
  });
}
...
function startNavigation() {
  ...
  stopLiveDrive();
  if (!pausedAt) {
    startedAt = performance.now() - latestProgress * durationMs;
  } else {
...
function handleLivePosition(position) {
  ...
  distanceValue.textContent = (totalLineDistance(livePoints) / 1000).toFixed(1);
  etaValue.textContent = livePoints.length < 2 ? "--" : String(livePoints.length);
...
function tick(now) {
  latestProgress = Math.min(1, (now - startedAt) / durationMs);
